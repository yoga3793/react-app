...
app.post('/api/product', (req, res) => {
    return  res.json(data.products.filter(x=> x._id == req.body.product_id));
});
...
app.post('/api/get-cart', (req, res) => {
  ...
  for (var i = 0; i < data.products.length; i++) {
    id = data.products[i]._id.toString();
...
app.post('/api/login', (req,res) => {
  ...
  if (user.length){
      ...
      let token_payload = {email: user[0].email, password: user[0].password};
